...
class TestRDFTXC(unittest.TestCase):
    def test_various_xc_combinations(self):
        ...
        mf_dh = dh.DH(mol, xc=(xc_scf_token, xc_eng_token)).run()
        print(mf_dh.e_tot)
    ...
    def test_complicated_xc_scf(self):
        ...
        mf_dh = dh.DH(mol, xc=xc_scf_token, force_evaluate=True).run()
        mf_scf = dft.RKS(mol, xc=xc_scf_token).density_fit().run()
