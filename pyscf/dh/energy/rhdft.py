...
def get_energy_purexc(xc_lists, rho, weights, restricted, numint=None):
    ...
    To evaluate exact exchange (HF or LR_HF), use ``get_energy_restricted_exactx``.
...
if __name__ == '__main__':
    ...
    def main_2():
        ...
        mol = gto.Mole(atom="O; H 1 0.94; H 1 0.94 2 104.5").build()
        mf_s = scf.RHF(mol)
        mf = RHDFT(mf_s, xc="B3LYPg").run()
        np.random.seed(0)
        nocc, nvir = mf.nocc, mf.nvir
        dmX = np.random.randn(3, nocc, nvir)
        mo_occ = mf.mo_occ
        idx_occ = mo_occ > 0
        idx_vir = ~idx_occ
        log = lib.logger.new_logger(verbose=5)
        # first epoch should be a lot slower, since mf.scf.gen_response will cache xc kernel first
        for epoch in range(3):
            t0, t1 = lib.logger.process_clock(), lib.logger.perf_counter()
            ax = mf.Ax0_Core_resp(idx_occ, idx_vir, idx_occ, idx_vir)(dmX)
            lib.logger.timer(log, f"Epoch {epoch} of Ax0_Core_resp", t0, t1)
